"""
NeuralTwin AI Facade
====================

Provides a **single, clean interface** for the Backend team to consume every
AI capability offered by NeuralTwin, without touching internal technical
...
* **Encapsulation** – callers never import anything from ``infrastructure``,
  ...
  plain Pydantic models defined in ``llm_engineering.domain``.
...
* **Config-driven** – automatically selects the correct LLM backend (Ollama /
  vLLM / OpenAI / Mock) based on ``llm_engineering.settings``.
"""
...
from llm_engineering.domain.inference import (
    ReasoningStep,
...
if TYPE_CHECKING:
    from llm_engineering.application.agents.research_agent import AgentResult
...
_REASONING_SYSTEM_PROMPT: str = (
    "You are NeuralTwin — an expert AI reasoning engine.\n\n"
...
class NeuralTwinAIFacade:
    """
    Unified entry-point for all NeuralTwin AI capabilities.

    Usage from the Backend / API layer::

        facade = NeuralTwinAIFacade()
    ...
    def __init__(self) -> None:
        ...
        logger.info(
            "NeuralTwinAIFacade initialised  "
